...
")
  ...
  defp parse(input) do
    ...
    {Enum.map(instructions, &String.split(&1, "|", trim: true)),
     Enum.map(updates, &String.split(&1, ",", trim: true))}
  end
  ...
  defp correct_order?([first | rest], instructions) do
    ...
    Enum.all?(rest, fn element -> !MapSet.member?(instruction, element) end) &&
      correct_order?(rest, instructions)
  end
